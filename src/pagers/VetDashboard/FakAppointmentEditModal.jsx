...
const FakAppointmentEditModal = ({
  ...
  const handleUpdateAppointment = () => {
    setIsLoading(true);
    let prevData = allAppointmentList;
    const indexOfElementToUpdate = prevData.findIndex(
      (element) => element._id === singleData?._id
    );

    // Check if the element was found
    if (indexOfElementToUpdate !== -1) {
      // Perform your updates here
      prevData[indexOfElementToUpdate].appointmentDate =
        moment(appointmentDateTime);
      prevData[indexOfElementToUpdate].appointmentTime =
        moment(appointmentDateTime).format("HH:mm");
      // Update other properties as needed
    }
    setAllAppointmentList(prevData);
...
export default FakAppointmentEditModal;
